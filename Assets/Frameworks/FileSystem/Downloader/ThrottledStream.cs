...
namespace Born2Code.Net
{
    ...
    public class ThrottledStream : Stream
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            int byteCount = _baseStream.Read(buffer, offset, count);
            if (byteCount > 0)
            {
        ...
        protected void Throttle(int bufferSizeInBytes2)
        {
            // Make sure the buffer isn't empty.
            if (_maximumBytesPerSecond <= 0 || _byteCount <= 0)
            {
                return;
            }

...
{
	abstract public class HorizontalScrollArea : Area
	{
		public HorizontalScrollArea(int layer) : base(layer)
		{
		}
		...
		protected virtual void InitDatas(int dataCounts, float entryWidth, float entryHeight, Vector2 margin)
		{
			...
			m_amountPerLine = Mathf.Max(1, (int)((height/* - m_margin.y*/) / (m_entryHeight + m_margin.y)));
			m_lineCount = Mathf.CeilToInt(1f * m_dataCount / m_amountPerLine);

			m_datasWidth = m_lineCount * (m_entryWidth + m_margin.x) + m_margin.x;
			m_dragging = false;
			m_offsetDeltaQueue.Clear();

			m_offset = moreThanOneView ? Mathf.Clamp(m_offset, 0f, m_datasWidth - width) : 0f;
		}
		...
		protected bool moreThanOneView { get { return m_datasWidth > width; } }
		...
		protected override void OnDrawGUI(Event evt)
        {
			...
			int startLine = Mathf.Max(0, Mathf.FloorToInt(m_offset / (m_entryWidth + m_margin.x)));
			int endLine = Mathf.Min(m_lineCount - 1, Mathf.FloorToInt((m_offset + width) / (m_entryWidth + m_margin.x)));
		...
		protected virtual Rect GetItemRect(int index)
		{
			...
			return new Rect(col * (m_entryWidth + m_margin.x) + m_margin.x - m_offset, row * (m_entryHeight + m_margin.y) + m_margin.y, m_entryWidth, m_entryHeight);
		}
		...
		protected override void OnDoInteraction(Event evt)
        {
			...
			if (!m_dragging)
			{
				...
				foreach (Rect entryRect in m_entryRects)
				{
					...
					{
						int startIdx = Mathf.Max(0, Mathf.FloorToInt(m_offset / m_entryWidth));
				...
				if (evt.type == EventType.ScrollWheel && localRect.Contains(evt.mousePosition))
				{
					if (moreThanOneView)  // 数据超过一屏
					{
						...
						m_offset = Mathf.Clamp(m_offset, 0f, m_datasWidth - width);
					}
					evt.Use();
				}
			}
			else
			{
				...
				{// 释放鼠标，处理列表归位
					...
					if (moreThanOneView)  // 数据超过一屏
					{
						if (m_offset < 0) tweener = Tweener.Create(m_offset, 0f, 0.2f, (x)=>m_offset=x, Ease.QuartOut);
						else if (m_offset > m_datasWidth - width) tweener = Tweener.Create(m_offset, m_datasWidth - width, 0.2f, (x)=>m_offset=x, Ease.QuartOut);
					}
					else // 数据不足一屏
					{
						if (m_offset != 0) tweener = Tweener.Create(m_offset, 0f, 0.2f, (x)=>m_offset=x, Ease.QuartOut);
				...
				{// 处理拖拽
					//记录拖拽速度队列
					float m_offsetDelta = m_lastMousePosi.x - evt.mousePosition.x;
					bool outOfBorder = moreThanOneView ? (m_offset < 0 || m_offset > m_datasWidth - width) : (m_offset != 0);
					...
					{// 拖拽中释放处理惯性
						...
						{
							tweener = Tweener.Create(m_offsetInertia, 0f, 0.5f, (x)=>
							{
								...
								else
								{
									...
									if (m_offset <= 0 || m_offset >= m_datasWidth - width)
									{
										m_offsetInertia = 0f;
										m_offset = Mathf.Clamp(m_offset, 0f, m_datasWidth - width);
									}
